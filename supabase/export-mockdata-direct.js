// Script to export mock data directly to JSON files
...
// Mock data (copied from mockData.ts)
const user_ids = {
...
fs.writeFileSync(path.join(dataDir, 'firebase-users.json'), JSON.stringify(users, null, 2));
fs.writeFileSync(path.join(dataDir, 'firebase-simcards.json'), JSON.stringify(simCards, null, 2));
fs.writeFileSync(path.join(dataDir, 'firebase-packages.json'), JSON.stringify(packages, null, 2));
fs.writeFileSync(path.join(dataDir, 'firebase-transactions.json'), JSON.stringify(transactions, null, 2));

console.log('Data exported and saved to JSON files:');
console.log('- users:', users.length, 'records');
console.log('- sim cards:', simCards.length, 'records');
